...
def main():
    ...
    count = 0  # 누적 폭탄수
    NUM_OF_BOMBS = 20  # 설치 희망 폭탄수
    while count < NUM_OF_BOMBS:  # NUM_OF_BOMBS 만큼 배치하도록 조건 지정
        # 무작위로 xpos,ypos 지정
        xpos = randint(0, WIDTH - 1)
        ypos = randint(0, HEIGHT - 1)

        if field[ypos][xpos] == EMPTY:  # 비어 있으면:
            field[ypos][xpos] = BOMB  # 폭탄 배치
            count += 1  # 폭탄수 증가
...
###
'''
1. 폭탄 수 조정하기: 만들어지는 폭탄수를 10개 더 추가하려면 어떻게 해야 할까요?
2. 화면 크기 변경하기: 게임의 가로 너비를 25 칸으로 늘리려면 어떤 코드를 수정해야 할까요?
3. 프레임 속도 조정하기: 게임의 프레임 속도를 초당 30 프레임으로 변경하려면 어디를 수정해야 할까요?
4. 폭탄 그리기 변경하기: 폭탄을 그리는 원의 크기를 더 크게 하려면 어떤 부분을 수정해야 할까요?
5. 게임 종료 조건 추가하기: 사용자가 게임 창을 닫을 때 이외에도, 특정 키(예: ESC 키)를 눌러 게임을 종료하려면 어떤 코드를 추가해야 할까요?
'''
